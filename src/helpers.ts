...
export function setPlayer(col: number, row: number, board: Array<Array<types.Space>>, player: types.Player | undefined) {
  const boardWidth = board[0].length;
  const boardHeight = board.length;
  if (col < 0 || col >= boardWidth || row < 0 || row >= boardHeight) return board;
  board[row][col].player = player;
  return board;
}

export function increase(col: number, row: number, board: Array<Array<types.Space>>) {
  const boardWidth = board[0].length;
  const boardHeight = board.length;
  if (col < 0 || col >= boardWidth || row < 0 || row >= boardHeight || board[row][col].value >= 4) return board;
  board[row][col].value++;
  return board;
}
...
export function initializeBoardVariant(variant: string, playerCount: number, board: types.Space[][], players: types.Player[]) {
    const boardWidth = board[0].length;
    const boardHeight = board.length;
    if (variant == "pickaxe") {
...
export function renderCycles(cycles: Array<Array<types.Space>>, callback: ()=>void, game: Game, callNextPlayer: boolean=true) {
...
export function calculateCycles(board: Array<Array<types.Space>>): Array<Array<types.Space>> {
  const squaresToSplit: Array<types.Space> = board.flat().filter(space => space.value == 4);
  for (let i = 0; i < squaresToSplit.length; i++) board = split(squaresToSplit[i].col, squaresToSplit[i].row, board);
  if (board.flat().filter(space => space.value == 4).length > 0) return [squaresToSplit].concat(calculateCycles(structuredClone(board)));
  return [squaresToSplit];
}

export function  calculateCycleResponse(cycles: Array<Array<types.Space>>, board: types.Board) {
...
export function  isGameOver(board: types.Board, player: types.Player, opponent: types.Player): boolean {
  return getAllOfPlayer(board, opponent).length == 0 || getAllOfPlayer(board, player).length == 0;
}

export function  getAllOfPlayer(board: types.Board, player: types.Player): Array<types.Space> {
...
export function  staticEval(board: types.Board, player: types.Player, opp: types.Player): number {
...
export function  maxPossibleStaticEval(board: types.Board, player: types.Player, opp: types.Player): number {
  return getAllOfPlayer(board, player).reduce((acc, cur)=>acc+cur.value, 0) + getAllOfPlayer(board, opp).reduce((acc, cur)=>acc+cur.value, 0) + getAllOfPlayer(board, player).length + getAllOfPlayer(board, opp).length;
}

export function  minimax(position: types.Board, depth: number, isMax: boolean, maxPlayer: types.Player, minPlayer: types.Player, alpha: number, beta: number, difficulty: number): [number, Array<types.Space>] {
  ...
  if (isMax) {
    ...
    const possibleMoves: Array<types.Space> = getAllOfPlayer(position, maxPlayer);
    const outcomes: Array<Array<Array<types.Space>>> = possibleMoves.map((cell: types.Space) => checkResponse(position, cell));
    ...
    let bestMoveSequence: Array<types.Space> = [];
    const possibleMoves: Array<types.Space> = getAllOfPlayer(position, minPlayer);
    const outcomes: Array<Array<Array<types.Space>>> = possibleMoves.map((cell: types.Space) => checkResponse(position, cell));

    for (let i = 0; i < outcomes.length; i++) {
      const childEval = minimax(outcomes[i], depth - 1, true, maxPlayer, minPlayer, alpha, beta, difficulty);
...
export function  evalBarWidth(game: Game) {
  ...
  return Math.round(getAllOfPlayer(game.board, game.players[0]).length/(getAllOfPlayer(game.board, game.players[0]).length+getAllOfPlayer(game.board, game.players[1]).length)*100)+'%';
}
